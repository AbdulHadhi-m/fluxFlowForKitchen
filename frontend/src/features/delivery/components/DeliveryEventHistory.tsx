        ...
        <div className="space-y-4 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-800">
          {events.map((evt) => (
            <div key={evt.id} className="flex items-start gap-4 relative pl-8">
              <span className="w-2.5 h-2.5 rounded-full bg-amber-500 absolute left-[7px] top-1.5 ring-4 ring-slate-100 dark:ring-slate-900" />
              <div className="flex-1 bg-slate-100/80 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800/80 p-3 rounded-2xl">
                ...
                {evt.notes && <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">{evt.notes}</p>}
                <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1.5">
                  Actor: <span className="text-slate-600 dark:text-slate-300 font-semibold">{evt.actor_name}</span>
                </p>
              </div>
            </div>
          ))}
