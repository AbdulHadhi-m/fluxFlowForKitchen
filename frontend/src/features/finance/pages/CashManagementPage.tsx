    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Cash Drawer Sessions & Payouts</h1>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {isLoading ? (
          ...
          <div className="col-span-full p-12 text-center text-slate-500 text-sm bg-slate-100/70 dark:bg-slate-900/40 rounded-2xl border border-slate-200 dark:border-slate-800">
          ...
          sessions.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className="p-5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 backdrop-blur-md space-y-4 hover:border-slate-300 dark:hover:border-slate-700 transition-all flex flex-col justify-between"
              >
                <div className="space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-bold text-slate-900 dark:text-white text-base">{s.register_name}</h3>
                      <p className="text-xs text-slate-500 dark:text-slate-400">Opened by <strong className="text-slate-600 dark:text-slate-300">{s.opened_by_name || "Cashier"}</strong></p>
                    </div>
                    <span
                  ...
                  {!isOpen && hasVariance && (
                    <div className={`p-2.5 rounded-lg border flex items-center justify-between text-xs ${
                      variance < 0 ? "bg-rose-500/10 border-rose-500/20 text-rose-600 dark:text-rose-300" : "bg-emerald-500/10 border-emerald-500/20 text-emerald-600 dark:text-emerald-300"
                    }`}>
                      ...
                      {s.status === "RECONCILIATION_REQUIRED" && (
                        <button
                          ...
                          className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/20 text-amber-600 dark:text-amber-300 hover:bg-amber-500/30"
                        >
                          Approve
                        </button>
                      )}
                ...
                {isOpen ? (
                  <div className="flex items-center gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1 py-1.5 px-3 rounded-lg text-xs font-semibold bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-300 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white transition-colors"
                    >
