...
export const AuditDetailModal: React.FC<AuditDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden text-slate-100 p-6 space-y-4 max-h-[90vh] flex flex-col">
        ...
        <div className="space-y-4 overflow-y-auto pr-1 flex-1">
          {/* Metadata Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 bg-slate-950 p-3 rounded-xl border border-slate-800/80 text-xs">
            <div>
            ...
            <div>
              ...
              <span className="font-bold text-white">
                {log.entity_type} <span className="text-slate-400 text-[10px]">({log.entity_id || "—"})</span>
              </span>
            </div>
            ...
            {log.correlation_id && (
              <div className="col-span-2">
                ...
                <span className="font-mono text-[11px] text-slate-400 truncate flex items-center gap-1">
          ...
          <div className="space-y-1">
            <span className="text-xs font-bold text-slate-300">Action Summary</span>
          ...
          {(Object.keys(log.before_data || {}).length > 0 || Object.keys(log.after_data || {}).length > 0) && (
            <div className="space-y-1.5">
              <span className="text-xs font-bold text-slate-300">State Snapshot Changes</span>
        ...
        <div className="flex justify-end pt-3 border-t border-slate-800 shrink-0">
          <Button
            ...
            variant="outline"
            onClick={onClose}
            className="border-slate-800 text-slate-400 hover:bg-slate-800 hover:text-white text-xs"
          >
            Close
          </Button>
        </div>
