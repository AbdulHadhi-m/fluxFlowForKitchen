...
export const StaffDisableDialog: React.FC<StaffDisableDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md shadow-2xl p-6 text-slate-100 space-y-4">
        <div className="flex items-center gap-3 text-rose-400">
          <div className="p-2 rounded-xl bg-rose-500/10 border border-rose-500/20">
            <AlertTriangle className="h-6 w-6" />
          </div>
          <div>
            <h3 className="text-base font-semibold text-white">Disable Staff Member</h3>
            <p className="text-xs text-slate-400">Security Access Revocation</p>
          </div>
        </div>

        <p className="text-xs text-slate-300 leading-relaxed">
          Are you sure you want to disable{" "}
          <span className="font-semibold text-white">{staff.display_name}</span> (
          <span className="font-mono text-slate-400">{staff.employee_id}</span>)?
        </p>

        <div className="p-3 rounded-lg bg-slate-950/80 border border-slate-800 text-[11px] text-slate-400 space-y-1">
          <p className="font-medium text-rose-300">&bull; Immediate Access Block</p>
          <p>
