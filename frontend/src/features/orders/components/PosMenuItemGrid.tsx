...
export const PosMenuItemGrid: React.FC<PosMenuItemGridProps> = ({ items, onSelectItem }) => {
  ...
  if (availableItems.length === 0) {
    return (
      <div className="py-20 text-center border border-dashed border-slate-800 rounded-2xl bg-slate-900/30 flex-1">
        <UtensilsCrossed className="h-10 w-10 text-slate-600 mx-auto mb-3" />
        <h3 className="text-sm font-medium text-slate-300">No available menu items</h3>
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 flex-1 overflow-y-auto max-h-[calc(100vh-220px)] pr-1">
      {availableItems.map((item) => (
        <button
          ...
          className="group flex flex-col justify-between p-3.5 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-blue-500/50 hover:bg-slate-800/40 text-left transition-all backdrop-blur-sm"
        >
          <div>
            <div className="text-xs font-semibold text-white group-hover:text-blue-300 transition-colors line-clamp-1">
              {item.name}
            </div>
            {item.description && (
          ...
          <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-800/60 w-full">
