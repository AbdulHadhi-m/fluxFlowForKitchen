      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden text-slate-100 p-6 space-y-4">
          ...
          <div>
            ...
            <span
              className={`font-mono font-black text-sm ${
                remaining < 0 ? "text-rose-500 font-bold" : "text-slate-200"
              }`}
            >
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-3.5 pt-1">
          <div className="space-y-1">
            <label className="text-xs text-slate-300">Wasted Quantity ({item.unit})</label>
            <Input
              type="number"
              step="0.001"
              max={current}
          ...
          <div className="space-y-1">
            <label className="text-xs text-slate-300">Wastage / Spoilage Reason</label>
            <select
              {...register("reason")}
