  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-100/70 dark:bg-slate-950/40">
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-300 dark:border-slate-700">
                {stockCount.status}
              </span>
            </div>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 font-medium">
                {stockCount.items.map((ci) => {
                  ...
                  return (
                    <tr key={ci.id} className="hover:bg-slate-200/70 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        {isEditable ? (
                          <input
                            ...
                            className="w-24 px-2 py-1 rounded bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 font-mono text-xs focus:border-emerald-500"
                          />
                        ) : (
                      ...
                      <td className="py-3 px-4">
                        {isEditable ? (
                          <input
                            ...
                            className="w-full px-2 py-1 rounded bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs"
                          />
                        ) : (
                          <span className="text-slate-500 dark:text-slate-400">{ci.notes || '—'}</span>
                        )}
                      </td>
                    </tr>
                  );
        ...
        {/* Footer Actions */}
        <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-slate-100/70 dark:bg-slate-950/40 flex items-center justify-between">
          <button
            type="button"
            onClick={onClose}
            ...
            {isEditable && (
              <>
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-300 dark:hover:bg-slate-700 text-sm font-semibold transition-colors"
                >
                  Save Progress
                </button>
                <button
