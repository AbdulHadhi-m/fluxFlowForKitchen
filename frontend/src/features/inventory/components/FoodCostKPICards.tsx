...
export const FoodCostKPICards: React.FC<FoodCostKPICardsProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      {/* 1. Total Valuation */}
      <div className="p-4 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 backdrop-blur-md relative overflow-hidden group hover:border-slate-300 dark:hover:border-slate-700 transition-all">
      ...
      <div className="p-4 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 backdrop-blur-md relative overflow-hidden group hover:border-slate-300 dark:hover:border-slate-700 transition-all">
        ...
        <div className="mt-3">
          <div className={`text-2xl font-bold ${Number(variance?.net_variance_cost || 0) > 0 ? 'text-rose-400' : 'text-slate-900 dark:text-slate-100'}`}>
        ...
        <div className="mt-3 flex items-baseline gap-2">
          ...
          {outOfStockCount > 0 && (
            <>
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">out of stock</span>
            </>
          )}
        </div>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Below minimum threshold levels</p>
