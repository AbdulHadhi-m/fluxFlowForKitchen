      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950/40">
          <div>
            ...
            <p className="text-xs text-slate-400">First-Expiry First-Out batch allocations</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {isLoading ? (
            <div className="p-8 text-center text-sm text-slate-400">Loading batch lots...</div>
          ) : batches.length === 0 ? (
                        ...
                        {batch.expiry_date && (
                          <span
                            className={`flex items-center gap-1 font-medium ${
                              isExpiringSoon ? 'text-amber-400' : 'text-slate-300'
                            }`}
                          >
