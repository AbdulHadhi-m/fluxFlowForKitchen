    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs">
          <span className="text-slate-400 block">Total Est. Purchase Requisition</span>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800/60 font-medium">
              {isLoading ? (
                ...
                suggestions.map((s) => (
                  <tr key={s.item_id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="py-3.5 px-4">
                      <div className="font-semibold text-slate-200 text-sm">{s.item_name}</div>
