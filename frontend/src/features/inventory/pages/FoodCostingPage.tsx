    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-md">
          ...
          <p className="text-xs text-slate-400 mt-1">Expected ingredient usage from completed orders</p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-md">
          ...
          <p className="text-xs text-slate-400 mt-1">Deducted stock movements + wastage + spoilage</p>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-md">
          ...
          <p className="text-xs text-slate-400 mt-1">Unaccounted portioning loss / spoilage gap</p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-800/60 font-medium">
                {isVarianceLoading ? (
                  ...
                  variance.items.map((row) => {
                    ...
                    return (
                      <tr key={row.item_id} className="hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="py-3.5 px-4 font-mono font-bold">
                          <span className={varCost > 0 ? 'text-rose-400' : 'text-slate-400'}>
                            ${varCost.toFixed(2)}
                          </span>
                        </td>
        ...
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-md space-y-4">
          ...
          <div className="space-y-2">
            {Object.entries(valuation?.by_location || {}).map(([loc, val]) => (
              ...
              >
                <span className="font-semibold text-slate-300">{loc.replace('_', ' ')}</span>
          ...
          <div className="space-y-2">
            {Object.entries(valuation?.by_type || {}).map(([itype, val]) => (
              <div
                ...
                className="flex items-center justify-between p-3 rounded-xl bg-slate-950/50 border border-slate-800/60 text-xs"
              >
                <span className="font-semibold text-slate-300">{itype.replace('_', ' ')}</span>
                <span className="font-mono font-bold text-emerald-400">
                  ${Number(val).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
