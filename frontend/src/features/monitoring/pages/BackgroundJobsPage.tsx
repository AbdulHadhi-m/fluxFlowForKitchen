    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-black tracking-tight text-white">Background Jobs</h1>
        </div>
      ...
      {jobs && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {jobs.stuck.recent.map((task, idx) => (
                    <div key={idx} className="flex items-center justify-between">
                      <div className="min-w-0">
                        <div className="text-xs text-slate-200 font-medium truncate">{task.task_name}</div>
            ...
            <Card className="bg-slate-900/60 border-slate-800 p-4">
              <div className="mb-3">
                <h3 className="text-sm font-bold text-white">Broker & Worker Health</h3>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-300">Worker availability</span>
                  <HealthBadge status={jobs.workers.status} />
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-300">Queue status</span>
