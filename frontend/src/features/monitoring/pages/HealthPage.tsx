    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-black tracking-tight text-white">System Health</h1>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(health.dependencies).map(([name, dep]) => (
              <Card key={name} className="bg-slate-900/60 border-slate-800 p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-white capitalize">{name.replace("_", " ")}</h3>
                ...
                <dl className="space-y-1.5">
                  {dep.details && (
                    <div className="flex justify-between">
                      ...
                      <dd className="text-[11px] text-slate-300">{dep.details}</dd>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <dt className="text-[11px] text-slate-500">Latency</dt>
                    <dd className="text-[11px] text-slate-300">{dep.latency_ms}ms</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-[11px] text-slate-500">Critical</dt>
                    <dd className="text-[11px] text-slate-300">{dep.critical ? "Yes" : "No"}</dd>
                  </div>
                  {dep.error && (
                    <div className="mt-2 rounded-lg bg-rose-500/5 border border-rose-500/20 p-2">
                      <span className="text-[11px] text-rose-300 break-all">{dep.error}</span>
                    </div>
                  )}
                </dl>
              </Card>
