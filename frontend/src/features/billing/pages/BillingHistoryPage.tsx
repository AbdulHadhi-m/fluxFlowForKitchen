  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-6 flex flex-col justify-between">
      <div className="max-w-7xl mx-auto w-full space-y-6 flex-1 flex flex-col">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-900/50 p-3 rounded-2xl border border-slate-800">
          <div className="relative w-full sm:w-72">
            ...
            <Input
              ...
              className="bg-slate-950 border-slate-800 pl-9 text-xs text-slate-200"
            />
          </div>
        ...
        {/* Invoices Table */}
        <div className="rounded-2xl bg-slate-900 border border-slate-800 overflow-hidden shadow-xl flex-1 flex flex-col">
                ...
                {isLoadingBills ? (
                  ...
                  bills.map((bill) => (
                    <tr key={bill.id} className="hover:bg-slate-800/40 transition-colors">
                      <td className="p-3.5 font-bold text-white">{bill.bill_number}</td>
                      <td className="p-3.5 font-sans">
                        <span className="font-mono text-slate-200 font-bold">{bill.order_number}</span>
                        <span className="text-slate-500 text-[11px] block">
                          {bill.table_name ? `Table ${bill.table_name}` : "Takeaway"}
                        </span>
                      </td>
                      <td className="p-3.5 font-bold text-white">${bill.grand_total}</td>
                      <td className="p-3.5 text-emerald-400 font-bold">${bill.total_paid}</td>
                      <td className="p-3.5">
                        <span className={`font-bold ${parseFloat(bill.balance_due) > 0 ? "text-amber-400" : "text-slate-400"}`}>
                          ${bill.balance_due}
                        </span>
                      </td>
                      <td className="p-3.5 font-sans">
                      ...
                      <td className="p-3.5 text-right font-sans">
                        <div className="flex items-center justify-end gap-1.5">
                          <Button
                            ...
                            className="h-8 border-slate-800 hover:bg-slate-800 text-slate-300 text-xs gap-1"
                          >
