      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden text-slate-900 dark:text-slate-100 p-6 space-y-4">
        ...
        <div className="bg-white dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800 flex justify-between items-center text-xs">
          <div>
            ...
            <span className="font-mono font-bold text-slate-900 dark:text-white">{bill.order_number}</span>
          </div>
          <div>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 pt-1">
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-slate-600 dark:text-slate-300">Tender Method</label>
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-2.5 rounded-xl border transition-all text-xs font-semibold gap-1.5 ${
                      isSelected
                        ? "bg-amber-500/10 border-amber-500/50 text-amber-600 dark:text-amber-300 shadow-md shadow-amber-500/10"
                        : "bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
                    }`}
                  >
            ...
            <div className="p-3 bg-slate-100 dark:bg-slate-950/80 rounded-xl border border-slate-200 dark:border-slate-800/80 space-y-2">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className="bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-mono text-sm"
                  placeholder="0.00"
                />
              </div>

              <div className="flex justify-between items-center pt-1 text-xs">
                <span className="text-slate-500 dark:text-slate-400 font-medium">Change Returned:</span>
          ...
          {paymentMethod !== "CASH" && (
            <div className="space-y-1.5">
              <label className="text-xs text-slate-600 dark:text-slate-300">Transaction Reference / Last 4 Digits</label>
              <Input
                {...register("reference")}
