      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md shadow-2xl overflow-hidden text-slate-100 p-6 space-y-4">
        <div className="flex items-center justify-between pb-2 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-white">Tax Invoice / Receipt</h2>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div id="printable-receipt" className="bg-slate-950 p-5 rounded-2xl border border-slate-800 font-mono text-xs space-y-4">
          {/* Header */}
          <div className="text-center border-b border-dashed border-slate-800 pb-3">
            <h2 className="text-base font-black text-white">FLUXIFLOW RESTAURANT</h2>
            <p className="text-[11px] text-slate-400 mt-0.5">Official Tax Receipt</p>
            <div className="flex justify-between text-[11px] text-slate-400 pt-2">
              <span>Bill: <b className="text-white">{bill.bill_number}</b></span>
              <span>Order: <b className="text-white">{bill.order_number}</b></span>
            </div>
          ...
          <div className="space-y-2 border-b border-dashed border-slate-800 pb-3">
            {bill.items.map((item) => (
              <div key={item.id} className="flex justify-between items-start">
                ...
                <div className="text-white font-bold">${item.line_total}</div>
              </div>
            ))}
          </div>

          {/* Monetary Summary */}
          <div className="space-y-1.5 border-b border-dashed border-slate-800 pb-3 text-slate-300">
            <div className="flex justify-between">
              <span>Subtotal:</span>
              <span className="text-white font-bold">${bill.subtotal}</span>
            </div>
          ...
          <div className="space-y-1.5">
            <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Settled Tenders</div>
