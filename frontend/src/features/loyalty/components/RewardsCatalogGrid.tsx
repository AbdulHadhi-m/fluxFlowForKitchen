...
export const RewardsCatalogGrid: React.FC<{ rewards: Reward[] }> = ({ rewards }) => {
  if (rewards.length === 0) {
    return (
      <div className="py-8 text-center text-xs text-slate-500 bg-slate-900/40 rounded-2xl border border-slate-800">
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {rewards.map((r) => (
        <Card key={r.id} className="bg-slate-900/60 border-slate-800 hover:border-slate-700 transition-all">
