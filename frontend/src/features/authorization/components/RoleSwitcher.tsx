...
export const RoleSwitcher: React.FC = () => {
  ...
  if (!availableRoles || availableRoles.length <= 1) {
    return (
      <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-950/40 border border-blue-800/40 text-blue-300 text-xs">
        <ShieldCheck className="h-3.5 w-3.5 text-blue-400" />
        <span className="font-medium">{activeRole?.name || activeRole?.code || "Default Role"}</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative inline-block text-left">
      <Button
        ...
        className="h-8 px-2.5 bg-slate-900 border-slate-700 hover:bg-slate-800 text-xs text-slate-200 flex items-center gap-2"
      >
        {isSwitchingRole ? (
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-1.5 w-56 rounded-xl bg-slate-900 border border-slate-800 shadow-2xl z-50 p-1.5 space-y-1 text-xs">
            <div className="px-2.5 py-1.5 text-[11px] font-medium text-slate-400 border-b border-slate-800">
