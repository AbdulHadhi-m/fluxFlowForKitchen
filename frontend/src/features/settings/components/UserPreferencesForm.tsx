...
export const UserPreferencesForm: React.FC<UserPreferencesFormProps> = ({
  ...
  const onFormSubmit = async (data: PreferenceFormData) => {
    if (data.theme) {
      setTheme(data.theme.toLowerCase() as Theme);
    }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
                <Palette className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Theme Mode
              </label>
              <select
                {...register("theme")}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
                <Clock className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Time Display Format
              </label>
              <select
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-300 flex items-center gap-1">
                <LayoutGrid className="h-3 w-3 text-slate-500 dark:text-slate-400" /> Grid Density
              </label>
              <select
