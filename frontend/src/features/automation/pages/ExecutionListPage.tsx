  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {workflowId && (
          <span className="text-[10px] text-slate-500 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-2 py-1.5">
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-16 rounded-2xl bg-slate-100/70 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 animate-pulse" />
          ))}
        </div>
      ) : (executions || []).length === 0 ? (
        ...
        <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800/80">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/80 dark:bg-slate-900/80 text-[10px] uppercase tracking-wider text-slate-500">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 bg-slate-100/70 dark:bg-slate-900/40">
              {(executions || []).map((ex) => {
                const duration =
                ...
                return (
                  <tr key={ex.id} className="hover:bg-slate-200/70 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-[11px] text-slate-500 dark:text-slate-400 hidden md:table-cell">{ex.trigger}</td>
                    <td className="px-4 py-3 text-[11px] text-slate-500 dark:text-slate-400 hidden lg:table-cell">
                      {ex.started_at ? new Date(ex.started_at).toLocaleString() : "—"}
                    </td>
                    <td className="px-4 py-3 text-[11px] text-slate-500 dark:text-slate-400 hidden lg:table-cell">{duration}</td>
                    <td className="px-4 py-3 text-[11px] text-slate-500 dark:text-slate-400">{ex.attempt_count}</td>
