    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-400">Human approval gates raised by automated workflows</p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {(approvals || []).map((a) => (
            <div
              key={a.id}
              className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800/80 hover:border-violet-500/40 transition-all"
            >
              <div className="flex flex-wrap items-start gap-3">
                <div className="flex-1 min-w-[220px]">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm font-bold text-white">{a.workflow_name}</span>
                  ...
                  {a.reason && <p className="text-[11px] text-slate-400 mt-2">{a.reason}</p>}
                  {Number(a.amount) > 0 && (
                    <p className="text-[11px] text-slate-400 mt-1">
                      Amount: <span className="text-amber-300 font-bold">${a.amount}</span>
                    </p>
                  )}
                  {a.related_data && Object.keys(a.related_data).length > 0 && (
                    <pre className="mt-2 p-2 rounded-lg bg-slate-800/60 text-[10px] text-slate-500 font-mono whitespace-pre-wrap break-all">
                  ...
                  {a.status !== "PENDING" && a.response_note && (
                    <div className="mt-2 text-[11px] text-slate-400">
                      Response: <span className="text-slate-300">{a.response_note}</span>
                    </div>
                  )}
                </div>
                ...
                {a.status === "PENDING" && (
                  <div className="flex flex-col gap-2">
                    ...
                    <Link
                      ...
                      className="px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-[11px] font-bold transition-colors text-center"
                    >
                      View Run
                    </Link>
                  </div>
