...
export const MenuItemCard: React.FC<MenuItemCardProps> = ({
  ...
  return (
    <div className="group bg-slate-900/60 hover:bg-slate-900 border border-slate-800/80 hover:border-slate-700/80 rounded-2xl p-5 flex flex-col justify-between transition-all duration-200 shadow-sm hover:shadow-xl relative overflow-hidden">
      <div>
        ...
        {item.description && (
          <p
            onClick={() => onOpenDetails(item)}
            className="text-xs text-slate-400 mt-2 line-clamp-2 cursor-pointer"
          >
            {item.description}
          </p>
        )}
