  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-white text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-1">Connecting to Kitchen...</h2>
        <p className="text-slate-400 text-sm">Fetching real-time preparation status</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-24 pt-6">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        {/* Hero Card */}
        <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 mb-6 relative overflow-hidden shadow-2xl">
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-3xl p-6 mb-6 space-y-4">
          ...
          <div className="divide-y divide-slate-800">
            {order.items.map((item, idx) => (
              <div key={idx} className="py-3 flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-slate-300">${item.line_total}</span>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <Link
            to={`/r/${order.restaurant_slug}`}
            className="flex-1 py-3.5 rounded-2xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-white font-bold text-sm text-center transition-colors"
          >
            Order More Items
          </Link>
        </div>
