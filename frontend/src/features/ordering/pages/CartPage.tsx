  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 text-center">
        <div className="w-20 h-20 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-500 mb-6">
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-24 pt-6">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <h1 className="text-2xl sm:text-3xl font-extrabold text-white mb-6">Your Order Cart</h1>

        {/* Order Type Toggle */}
        <div className="bg-slate-900 border border-slate-800 p-1.5 rounded-2xl flex items-center gap-2 mb-6">
          <button
            type="button"
        ...
        {/* Cart Item List */}
        <div className="bg-slate-900/70 border border-slate-800 rounded-3xl p-6 mb-6 space-y-5">
          {items.map((item) => (
            <div
              key={item.menu_item_id}
              className="flex items-start justify-between gap-4 pb-5 border-b border-slate-800/80 last:border-0 last:pb-0"
            >
              ...
              {/* Quantity Controls */}
              <div className="flex items-center gap-3 bg-slate-950 border border-slate-800 px-2.5 py-1 rounded-xl">
                <button
                ...
                <span className="font-extrabold text-sm text-white w-5 text-center">
                  {item.quantity}
                </span>
                <button
                  onClick={() => updateQuantity(item.menu_item_id, item.quantity + 1)}
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-3xl p-6 mb-6">
          ...
          <textarea
            ...
            maxLength={300}
            className="mt-2 w-full px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 resize-none"
          />
        </div>
          ...
          {appliedCoupon ? (
            ...
            <form onSubmit={handleApplyCoupon} className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white placeholder-slate-500 uppercase font-medium focus:outline-none focus:border-amber-500"
              />
              <button
                type="submit"
        ...
        {/* Totals Breakdown */}
        <div className="bg-slate-900/70 border border-slate-800 rounded-3xl p-6 mb-8 space-y-3">
          ...
          <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
            <span className="text-lg font-bold text-white">Estimated Subtotal</span>
