...
export const SecurityEventsPage: React.FC = () => {
  ...
  const getSeverityBadge = (severity: string) => {
    switch (severity) {
      ...
      default:
        return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-700/40 text-slate-300 border border-slate-600/30">LOW</span>;
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black tracking-tight text-white">Security Event Log</h1>
          </div>
      ...
      <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 sm:grid-cols-4 gap-2.5 bg-slate-900/40 p-3 rounded-xl border border-slate-800/60">
              ...
              {loading ? (
                ...
                events.map((evt) => (
                  ...
                  >
                    ...
                    <td className="p-3.5 font-mono text-[11px] text-indigo-300 font-bold">
                      {evt.event_type}
                    </td>
                    ...
                    <td className="p-3.5 text-right">
                      <Button size="sm" variant="ghost" className="h-7 text-xs text-indigo-400 hover:text-white p-1">
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <Card className="bg-slate-900 border-slate-800 w-full max-w-lg p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <Shield className="h-4 w-4 text-indigo-400" />
                <h2 className="text-sm font-bold text-white">Security Event Detail</h2>
              </div>
              <Button size="sm" variant="ghost" onClick={() => setSelectedEvent(null)} className="h-6 w-6 p-0 text-slate-400 hover:text-white">
                ✕
              </Button>
            </div>
            ...
            <div className="space-y-2.5 text-xs">
              <div className="grid grid-cols-3 gap-2">
                ...
                <span className="col-span-2 font-mono text-indigo-300 font-bold">{selectedEvent.event_type}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <span className="col-span-2 text-slate-200">{new Date(selectedEvent.created_at).toUTCString()}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <span className="text-slate-400">User:</span>
                <span className="col-span-2 text-white font-medium">{selectedEvent.user_email || "System / Unauthenticated"}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <span className="text-slate-400">IP Address:</span>
                <span className="col-span-2 font-mono text-slate-300">{selectedEvent.ip_address || "—"}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <span className="text-slate-400">Correlation ID:</span>
                <span className="col-span-2 font-mono text-[11px] text-slate-400">{selectedEvent.correlation_id || "—"}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                <span className="text-slate-400">Description:</span>
                <span className="col-span-2 text-slate-200">{selectedEvent.description}</span>
              </div>
              {selectedEvent.metadata && Object.keys(selectedEvent.metadata).length > 0 && (
                <div className="space-y-1 pt-2 border-t border-slate-800">
                  <span className="text-slate-400 font-semibold">Metadata:</span>
                  <pre className="bg-slate-950 p-2 rounded text-[11px] font-mono text-slate-300 overflow-x-auto">
            ...
            <div className="flex justify-end pt-2">
              <Button size="sm" onClick={() => setSelectedEvent(null)} className="bg-slate-800 hover:bg-slate-700 text-white text-xs">
                Close
              </Button>
            </div>
          </Card>
