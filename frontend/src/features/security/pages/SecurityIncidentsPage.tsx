...
export const SecurityIncidentsPage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "CONTAINED":
        return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">CONTAINED</span>;
      case "RESOLVED":
      ...
      default:
        return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-700/40 text-slate-300 border border-slate-600/30">CLOSED</span>;
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black tracking-tight text-white">Security Incidents</h1>
          </div>
      ...
      {/* Filters */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 bg-slate-900/40 p-3 rounded-xl border border-slate-800/60">
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-amber-500 h-9"
        >
        ...
        {loading ? (
          ...
          incidents.map((inc) => (
            <Card
              ...
              className="bg-slate-900/60 border-slate-800 p-4 hover:border-slate-700 transition-all cursor-pointer"
            >
              ...
              <p className="text-xs text-slate-400 mt-2 line-clamp-2">{inc.description || "No description provided."}</p>
              <div className="flex items-center gap-4 mt-3 pt-2 border-t border-slate-800/60 text-[11px] text-slate-500">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <Card className="bg-slate-900 border-slate-800 w-full max-w-md p-6 space-y-4 shadow-2xl">
      ...
      {selectedIncident && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <Card className="bg-slate-900 border-slate-800 w-full max-w-xl p-6 space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                {getStatusBadge(selectedIncident.status)}
                <h2 className="text-sm font-bold text-white">{selectedIncident.title}</h2>
              </div>
              <Button size="sm" variant="ghost" onClick={() => setSelectedIncident(null)} className="h-6 w-6 p-0 text-slate-400 hover:text-white">
                ✕
              </Button>
            </div>
              ...
              {selectedIncident.notes?.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-40 overflow-y-auto">
                  {selectedIncident.notes.map((n, i) => (
                    <div key={i} className="bg-slate-950/80 p-2 rounded border border-slate-800/60 text-[11px]">
            ...
            <div className="space-y-2 pt-2 border-t border-slate-800">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[11px] text-slate-400 block mb-1">Update Status</label>
                  <select
                    value={newStatus}
              ...
              <div>
                <label className="text-[11px] text-slate-400 block mb-1">Add Note</label>
                <textarea
                  rows={2}
                  value={noteText}
