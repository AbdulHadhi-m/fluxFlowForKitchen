  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto animate-in fade-in duration-300">
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-xl font-black tracking-tight text-white">Two-Factor Authentication (2FA)</h1>
      ...
      <Card className="bg-slate-900/60 border-slate-800 p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {mfaStatus?.mfa_enabled ? (
              ...
              <div className="h-10 w-10 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-400">
        ...
        {setupData && (
          <div className="border-t border-slate-800 pt-4 space-y-4 animate-in fade-in">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider">Step 1: Link Authenticator App</h3>
            ...
            <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex items-center justify-between">
              ...
              <Button size="sm" variant="ghost" onClick={copySecret} className="h-7 text-xs text-slate-400 hover:text-white gap-1">
                {copiedSecret ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5" />}
                {copiedSecret ? "Copied" : "Copy"}
              </Button>
            </div>

            <h3 className="text-xs font-bold text-white uppercase tracking-wider pt-2">Step 2: Enter 6-Digit Code</h3>
            <form onSubmit={handleVerify} className="space-y-3">
              <Input
                ...
                placeholder="123456"
                className="font-mono text-center tracking-widest text-lg bg-slate-950 border-slate-800 h-11"
              />
        ...
        {recoveryCodes.length > 0 && (
          <div className="border-t border-slate-800 pt-4 space-y-3 animate-in fade-in">
            ...
            <div className="grid grid-cols-2 gap-2 bg-slate-950 p-3 rounded-lg border border-slate-800 font-mono text-xs text-slate-300">
              {recoveryCodes.map((code, idx) => (
                <div key={idx} className="p-1 bg-slate-900/60 rounded text-center font-bold text-emerald-300">
                  {code}
                </div>
              ))}
            </div>
      ...
      {showDisableModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <Card className="bg-slate-900 border-slate-800 w-full max-w-md p-6 space-y-4 shadow-2xl">
            <h2 className="text-sm font-bold text-white flex items-center gap-2 text-rose-400">
