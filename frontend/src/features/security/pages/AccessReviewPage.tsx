      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black tracking-tight text-slate-900 dark:text-white">Access & Session Review</h1>
          </div>
        ...
        <div
          className={`p-3 rounded-xl text-xs flex items-center gap-2 ${
            notification.type === "success"
              ? "bg-emerald-950/40 border border-emerald-800 text-emerald-600 dark:text-emerald-300"
              : "bg-rose-950/40 border border-rose-800 text-rose-600 dark:text-rose-300"
          }`}
        >
          {notification.type === "success" ? (
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 text-slate-600 dark:text-slate-300">
              {loading ? (
                ...
                filteredUsers.map((user) => (
                  <tr key={user.user_id} className="hover:bg-slate-200/70 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="p-3.5">
                      <div className="font-bold text-slate-900 dark:text-white text-xs">{user.full_name || "—"}</div>
                    ...
                    <td className="p-3.5 text-right">
                      {user.active_sessions > 0 ? (
                        <Button
                          ...
                          className="h-7 text-xs text-rose-400 hover:text-rose-600 dark:hover:text-rose-300 hover:bg-rose-950/40 gap-1"
                        >
