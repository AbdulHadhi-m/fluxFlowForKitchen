      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/40">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Supplier Performance Scorecard</h2>
              <p className="text-xs text-slate-400">Quality, fulfillment, and reliability metrics</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl bg-slate-800/40 border border-slate-700/50">
                <div>
                  <h3 className="font-semibold text-white text-base">{scorecard.supplier_name}</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Terms: {scorecard.payment_terms}</p>
                </div>
              ...
              <div className="p-5 rounded-xl bg-gradient-to-br from-indigo-950/30 to-slate-900 border border-indigo-500/20">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-slate-400">Fulfillment Fill Rate</span>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-slate-800/30 border border-slate-800 space-y-1">
                  ...
                  <p className="text-lg font-bold text-white">{scorecard.total_delivered_quantity}</p>
                ...
                <div className="p-4 rounded-xl bg-slate-800/30 border border-slate-800 space-y-1">
                  ...
                  <p className="text-lg font-bold text-white">{scorecard.standard_lead_time_days} Days</p>
                ...
                <div className="p-4 rounded-xl bg-slate-800/30 border border-slate-800 space-y-1">
                  ...
                  <p className="text-lg font-bold text-white">{scorecard.completed_orders} / {scorecard.total_orders}</p>
                  <p className="text-xs text-slate-500">Lifetime purchase orders</p>
                </div>
              </div>
            </>
          ) : (
            <p className="text-center text-sm text-slate-400 py-8">No performance records found.</p>
          )}
        </div>
