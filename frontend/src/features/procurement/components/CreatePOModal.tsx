...
export const CreatePOModal: React.FC<CreatePOModalProps> = ({
  ...
  const suppliers = useMemo(() => propSuppliers || fetchedSuppliers || [], [propSuppliers, fetchedSuppliers]);
  const inventoryItems = useMemo(
    () => propInventoryItems || fetchedItems || [],
    [propInventoryItems, fetchedItems]
  );
  ...
  }, [requisition, prefillItem, inventoryItems, items.length]);
                ...
                >
                  <div className="flex-1">
                    <select
                      ...
                      className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 dark:text-white focus:border-emerald-500"
                      required
                    >
