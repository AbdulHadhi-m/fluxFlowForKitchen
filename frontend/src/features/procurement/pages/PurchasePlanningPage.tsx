  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md overflow-hidden">
        {isLoading ? (
          <div className="py-16 text-center text-slate-400 text-sm">Calculating inventory replenishment suggestions...</div>
        ) : recommendations && recommendations.length > 0 ? (
          <div className="divide-y divide-slate-800/60">
            {recommendations.map((rec) => (
              <div
                key={rec.inventory_item_id}
                className="p-5 hover:bg-slate-800/20 transition-colors flex flex-col lg:flex-row lg:items-center justify-between gap-4"
              >
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    <h3 className="font-bold text-white text-base">{rec.item_name}</h3>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400">
                    <span className="flex items-center gap-1">
                      ...
                      On-Hand: <strong className="text-white ml-1">{rec.current_stock} {rec.unit}</strong>
                    </span>
                    <span>Par Level: <strong className="text-slate-200">{rec.par_level} {rec.unit}</strong></span>
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      Preferred Vendor: <strong className="text-slate-200 ml-1">{rec.preferred_supplier_name}</strong>
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock className="w-3.5 h-3.5 text-slate-500" />
                      Lead Time: <strong className="text-slate-200 ml-1">{rec.lead_time_days} days</strong>
                    </span>
                  </div>
                </div>
