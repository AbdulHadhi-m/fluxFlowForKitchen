  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md overflow-hidden">
        {isLoading ? (
          <div className="py-16 text-center text-slate-400 text-sm">Loading requisitions...</div>
        ) : requisitions && requisitions.length > 0 ? (
          <div className="divide-y divide-slate-800/60">
            {requisitions.map((req) => (
              <div key={req.id} className="p-5 hover:bg-slate-800/20 transition-colors space-y-3">
                ...
                <div className="flex items-center gap-4 text-xs text-slate-400">
                  <span>Requested by: <strong className="text-slate-300">{req.requester_name}</strong></span>
                  <span>Location: <strong className="text-slate-300">{req.location}</strong></span>
                  {req.reason && <span>Reason: <em className="text-slate-300">"{req.reason}"</em></span>}
                </div>
                    ...
                    <tbody className="divide-y divide-slate-800/40">
                      {req.items.map((line) => (
                        <tr key={line.id}>
                          ...
                          <td className="py-1.5 text-slate-400 font-mono">{line.sku}</td>
