    ...
    <div className="space-y-6">
      ...
      {activeTab === "returns" ? (
        <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md overflow-hidden">
          {returnsLoading ? (
            <div className="py-16 text-center text-slate-400 text-sm">Loading purchase returns...</div>
          ) : returns && returns.length > 0 ? (
            <div className="divide-y divide-slate-800/60">
              {returns.map((ret) => (
                <div key={ret.id} className="p-5 hover:bg-slate-800/20 transition-colors space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-slate-400">Reason: <strong className="text-slate-200">{ret.reason}</strong></span>
                    </div>
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-400">
                    <span className="flex items-center gap-1">
                      ...
                      Vendor: <strong className="text-white ml-1">{ret.supplier_name}</strong>
                    </span>
                    <span>Requested by: <strong className="text-slate-300">{ret.requested_by_name}</strong></span>
                  </div>
                  ...
                  <div className="bg-slate-950/40 rounded-xl p-3 border border-slate-800/60">
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-slate-800/40">
                        {ret.items.map((line) => (
                          <tr key={line.id}>
                            <td className="py-1.5 font-medium text-white">{line.item_name}</td>
                            <td className="py-1.5 font-mono text-rose-400">
                              {line.quantity} {line.unit}
                            </td>
                            <td className="py-1.5 font-mono text-slate-400">${line.unit_cost}</td>
        ...
        <div className="rounded-2xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-md overflow-hidden">
          {creditsLoading ? (
            <div className="py-16 text-center text-slate-400 text-sm">Loading credits ledger...</div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {credits.map((cr) => (
                    <tr key={cr.id} className="hover:bg-slate-800/20">
                      <td className="p-4 font-mono font-bold text-emerald-400">{cr.credit_note_number}</td>
                      <td className="p-4 font-semibold text-white">{cr.supplier_name}</td>
                      ...
                      <td className="p-4 text-slate-400">{cr.issued_date}</td>
                      <td className="p-4 text-slate-300">{cr.reason}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
