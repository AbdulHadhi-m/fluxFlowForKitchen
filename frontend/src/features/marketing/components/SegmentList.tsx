  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between bg-slate-900/60 p-3 rounded-2xl border border-slate-800/80">
        <div>
          <h3 className="text-sm font-bold text-white">Dynamic Audience Segments</h3>
          <p className="text-xs text-slate-400">Target promotions and campaigns by customer spend and dining patterns</p>
        </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-40 rounded-2xl bg-slate-900/40 border border-slate-800 animate-pulse" />
          ))}
        </div>
      ) : segments.length === 0 ? (
        <div className="py-16 text-center rounded-2xl bg-slate-900/40 border border-dashed border-slate-800 p-8">
          ...
          <h4 className="text-sm font-bold text-white mb-1">No Customer Segments</h4>
          ...
          {segments.map((seg) => (
            ...
            >
              <div>
                ...
                <h4 className="text-sm font-bold text-white mb-1">{seg.name}</h4>
                {seg.description && <p className="text-xs text-slate-400 line-clamp-2 mb-3">{seg.description}</p>}

                <div className="p-2.5 rounded-xl bg-slate-950/60 border border-slate-800/50 space-y-1 text-[11px] mb-3">
                  {Number(seg.min_spend) > 0 && (
                    <div className="flex justify-between text-slate-400">
                      <span>Min Lifetime Spend:</span>
                      <span className="font-semibold text-white">${seg.min_spend}</span>
                    </div>
                  )}
                  {seg.min_visits > 0 && (
                    <div className="flex justify-between text-slate-400">
                      <span>Min Visits:</span>
                      <span className="font-semibold text-white">{seg.min_visits}+</span>
                    </div>
                  )}
                  {seg.inactive_days > 0 && (
                    <div className="flex justify-between text-slate-400">
                      <span>Inactive Threshold:</span>
                      <span className="font-semibold text-white">{seg.inactive_days} days</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="p-5 space-y-4">
              {isPreviewLoading ? (
                ...
                <>
                  <div className="p-3 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-between">
                    <div>
                      <div className="text-xs text-indigo-300 font-semibold">Total Audience Reach</div>
                  ...
                  <div>
                    <h5 className="text-xs font-bold text-slate-300 mb-2">Sample Profiles (Masked for Privacy)</h5>
              ...
              <div className="pt-3 border-t border-slate-800 flex justify-end">
                <button
                  ...
                  className="px-4 py-1.5 rounded-xl bg-slate-800 text-xs font-semibold text-slate-200 hover:bg-slate-700"
                >
                  Close Preview
                </button>
              </div>
