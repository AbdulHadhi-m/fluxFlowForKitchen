...
import { reportError, toErrorMessage } from "@/lib/monitor";
...
export class ErrorBoundary extends Component<Props, State> {
  ...
  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    try {
      ...
      // Reporting must never break the boundary
    }
  }
  ...
  public render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-[400px] flex flex-col items-center justify-center p-6 text-center space-y-4 bg-slate-100/70 dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-800 m-4">
